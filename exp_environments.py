...
class FL_mnist(gym.Env):
    ...
    def __init__(self, args):  # att_trainset, validset):
        ...
        self.observation_space = spaces.Box(low=low, high=high,
                                            shape=(int(args.num_clients * args.subsample_rate), 4))  # self.seed()
    ...
    def step(self, action):
        ...
        original_reward = self.loss - new_loss
        ...
        for i, cid in enumerate(self.cids):
            # <<< 替换代码 >>>
            # 原始代码是：sim_cg = score_cid[cid][3]
            # 原始代码是：spatial_outlier_score = 1.0 - sim_cg

            # 修正后：直接使用 FedInv Score 作为离群分数 (FedInv 分数越高代表越离群)
            fedinv_outlier_score = fedinv_scores[i]
            # <<< 结束替换 >>>

            # 获取分配给该客户端的权重
            assigned_weight = k[i].item()

            # 核心：给一个 FedInv 离群分数高的客户端越高的权重，受到的惩罚越大
            penalty += self.penalty_coefficient * fedinv_outlier_score * assigned_weight
